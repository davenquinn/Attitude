...
def trend_plunge(orientation, *args, **kwargs):
    ...
    # kwargs["linewidth"] = 0
    defaults = dict(linewidth=0)
    kwargs.update({k: kwargs.pop(k, v) for k, v in defaults.items()})
    ...
    for i, level in enumerate(levels):
        el = [N.degrees(i) for i in orientation.error_ellipse(vector=True, level=level)]
...
def strike_dip(orientation, *args, **kwargs):
    ...
    for i, level in enumerate(levels):
        el = [
            N.degrees(ell)
            for ell in orientation.error_ellipse(level=level, spherical=spherical)
        ]
