...
class SideViewComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    const xAx = d3.axisBottom(xA)
      .tickFormat(fmt)
      .tickSizeOuter(0);

    const _x = axes.append('g.x.axis')
      .translate([0,innerSize.height])
      .call(xAx);

    this.azLabel = _x.append('text.axis-label')
  ...
  updateAzimuth() {
    ...
    const hyp = hyperbolicErrors(angle, v_, x,y)
      .width(150)
      .nominal(false);
    ...
    // Grouped data
    const gp = this.singlePlanes.filter(d => d.group != null)
            .map(d => d.group);

    sel = this.errorContainerGrouped
      .selectAll('g.error-hyperbola')
      .data(gp);
    esel = sel.enter()
      .append('g.error-hyperbola');
