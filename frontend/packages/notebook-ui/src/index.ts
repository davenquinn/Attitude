...
class AttitudeUI extends React.Component {
  ...
  render() {
    ...
    const {azimuth, hovered, selection, showGroups, zoomedToSelection} = this.state;
    ...
    const selectionList = h(SelectionListComponent, {
      ...
      zoomedToSelection,
      zoomToSelection: () => {
        return this.setState({zoomedToSelection: !this.state.zoomedToSelection});
      },
      onToggleShowGroups: () => {
  ...
  getData() {
    ...
    if (!zoomedToSelection) { return attitudes; }
    return selection;
  }
  ...
  updateSelection(ids){
    ...
    for (let id of Array.from(ids)) {
      if ((id.members != null) && !this.props.allowGroupSelection) { continue; }
