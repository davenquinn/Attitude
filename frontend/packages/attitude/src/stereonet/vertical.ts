...
export default stereonet => (function(opts) {
  if (opts == null) { opts = {}; }
  if (opts.startOffset == null) { opts.startOffset = 10; }
  // correct for start at bottom
  opts.startOffset += 100;
  if (opts.labelPadding == null) { opts.labelPadding = 2; }
  let {dipLabels, nLabels} = opts;
  if (nLabels == null) { nLabels = 2; }
