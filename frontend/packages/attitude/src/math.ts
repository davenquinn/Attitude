...
const transpose = function(array, length=null) {
  ...
  for (let i = 0, end = array.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    for (let j = 0, end1 = length, asc1 = 0 <= end1; asc1 ? j < end1 : j > end1; asc1 ? j++ : j--) {
...
const ellipse = function(opts){
  ...
  const ellAdaptive = function(a,b){
    ...
    for (let i = 0, end = v, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
...
const cart2sph = function(opts){
  if (opts == null) { opts = {}; }
  if (opts.degrees == null) { opts.degrees = false; }
  if (opts.traditionalLayout == null) { opts.traditionalLayout = false; }
  if (opts.upperHemisphere == null) { opts.upperHemisphere = true; }
...
const planeErrors = function(axesCovariance, axes, opts){
  ...
  if (opts == null) { opts = {}; }
  if (opts.n == null) { opts.n = 100; }
  if (opts.upperHemisphere == null) { opts.upperHemisphere = true; }
  const sheet = opts.sheet || 'nominal';
  if (axes == null) { axes = identity; }
  if (opts.traditionalLayout == null) { opts.traditionalLayout = true; }
...
const normalErrors = function(axesCovariance, axes, opts){
  ...
  if (opts.upperHemisphere == null) { opts.upperHemisphere = true; }
  if (opts.traditionalLayout == null) { opts.traditionalLayout = true; }
  if (opts.sheet == null) { opts.sheet = 'upper'; }
  if (axes == null) { axes = identity; }
  if (opts.level == null) { opts.level = 1; }
  ...
  const stepFunc = function(es){
    ...
    return (() => {
      const result = [];
      for (let i of Array.from(axes)) {         result.push(sdot(e,i));
      }
      return result;
    })();
  };
...
var deconvolveAxes = function(axes){
  ...
  for (let i = 0, end = axes.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    for (let j = 0, end1 = axes.length, asc1 = 0 <= end1; asc1 ? j < end1 : j > end1; asc1 ? j++ : j--) {
      axes[j][i] /= sv[i];
    }
  }
  return [sv,axes];
